...
class PlaceService {
  ...
  Future<List<Place>> fetchPlaces(num userId, int cityId, int catId) async {
    final dio = Dio();
    dio.options.headers['Authorization'] = 'Bearer $token';

    dio.options.validateStatus =
        (status) => status! < 500; // يعتبر أي حالة أقل من 500 على أنها صحيحة

    try {
      final response = await dio
          .get('$url/api/user/getPlaces/${lang}/$userId/$cityId/$catId');
