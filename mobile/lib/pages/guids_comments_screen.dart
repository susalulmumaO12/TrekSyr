...
class GuidsCommentsScreen extends StatefulWidget {
   GuidsCommentsScreen({Key? key, required this.token, required this.id}) : super(key: key);
   final String token;
   final num id;
  @override
  State<GuidsCommentsScreen> createState() => _GuidsCommentsScreenState();
}

class _GuidsCommentsScreenState extends State<GuidsCommentsScreen> {
  List<Map<String, dynamic>> guidsCommentsList = [];

  @override
  void initState() {
    super.initState();
    fetchGuidsCommentsScreen(); // Default to fetching guides sorted by name
  }

  void fetchGuidsCommentsScreen() async {
    var data = await getGuidsComments(widget.token,widget.id);
    setState(() {
      guidsCommentsList = data.map<Map<String, dynamic>>((guidcomment) {
        ...
        return {
          'name': guidcomment['name'],
          'time': guidcomment['time'],
          'comment': guidcomment['comment'],
          'rate': guidcomment['rate'],
          'user_id': guidcomment['user_id'],
          'image': guidcomment['image'],
        };
      }).toList();
    });
  }
                            ...
                            Padding(
                              ...
                              child: SizedBox(
                                ...
                                child: ListView.builder(
                                  ...
                                  itemCount: guidsCommentsList.length,
                                  itemBuilder: ((context, index) {
                                    var guidcomment=guidsCommentsList[index];
                                  return CommentComponent(image: guidcomment['image'], name: guidcomment['name'], rate: guidcomment['rate'], comment: guidcomment['comment'], time: guidcomment['time'],);
                                })),
